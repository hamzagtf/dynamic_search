...
import AddNewData, { reformDataFromServer } from "./dynamic-search";
...
const reformData = (data, patient) =>
	data.map((i) => ({
		controlData: parseInt(i.id),
		controlDataId: i.controlDataId,
		patient,
		value: i.value,
		date: i.date,
	}));
...
function App() {
	...
	const save = (data) => {
		...
		const u_data = updated_data.length
			? reformData(updated_data, patientID)
			: null;

		// removed
		const removed = data.filter((j) => j.state === "removed");
		const deletedDataIds = removed.length ? removed.map((i) => i.controlDataId) : null; // deleted ids
		const deletedData = removed.length
			? reformData(removed, patientID)
			: null;
		...
		if (updated_data.length) {
			...
			const serverUpdatedData = u_data.map((i) => ({
				id: i.controlDataId,
				body: i
			}));
			console.log("updated data", u_data);
			serverUpdatedData.map((item) =>
				sendDataToServer(
					item.body,
					`http://127.0.0.1:8000/app/patient/${item.id}/`,
					"PUT"
				)
			);
		...
		if (removed.length) {
			...
			deletedDataIds.map((i) =>
				sendDataToServer(
					"",
					`http://127.0.0.1:8000/app/patient/${i}`,
					"DELETE"
				)
			);
	...
	useEffect(() => {
		...
		pData
			.then((i) => i.json())
			.then((n) => {
				setPatientData(n.map(reformDataFromServer));
			});
